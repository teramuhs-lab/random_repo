...
# -----------------------------------------------------------------------------
# Purpose  : Queries each SQL server for batches closed at an ADJUDICATION
#            location (LocationTypeID 18) yesterday. Results are grouped by
#            site and batch type, with a Grand Total row at the end.
...
# -------- SQL QUERY --------
# Batches closed at an ADJUDICATION location (LocationTypeID 18) yesterday.
# osLocationLocationType maps each LocationID to its type, so all adjudication
# locations are matched rather than a single hardcoded LocationID.
...
;WITH AdjudicatedBatches AS (
    ...
    JOIN dbo.osLocationLocationType llt WITH (NOLOCK) ON llt.LocationID = bh.LocationID
    JOIN dbo.osSite ost WITH (NOLOCK) ON ost.siteID = b.SiteID
    WHERE bh.BatchStatusID = 2 -- Closed
    AND llt.LocationTypeID = 18 -- Adjudication Functions
