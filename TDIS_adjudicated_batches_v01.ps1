...
# Shows 6 columns in HTML report
# ============================================
...
$HtmlFile = "$ReportFolder\TDIS_adjudicated_batches_$(Get-Date -Format yyyyMMdd_HHmmss).html"
...
function ConvertTo-HtmlSection {
    ...
    return @($Table) |
        Select-Object sitecode, SiteName,
            @{Name='BatchDate'; Expression={ if ($_.BatchDate) { ([datetime]$_.BatchDate).ToString('yyyy-MM-dd') } else { '' } }},
