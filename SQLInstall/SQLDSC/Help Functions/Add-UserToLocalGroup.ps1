...
#>
    ...
    (
        ...
        #[ValidateScript({$_ -match ('(?# The user name should follow the "DOMAIN\USER" convention. )^[A-Za-z0-9.]+\\[A-Za-z0-9.]+$')})]
        [string[]]$UserName,
	...
	Write-Progress -Activity 'Adding users to local groups' -Status 'Initializing' -Id 0
	...
	{
		Write-Progress -Activity 'Adding users to local groups on computer' -Status $computer -Id 0 -PercentComplete ($i / $computerCount * 100)

		$ii = 1
		foreach ( $localGroup in $LocalGroupName )
		{
			Write-Progress -Activity 'Adding users to the local group' -Status $localGroup -Id 1 -ParentId 0 -PercentComplete ($ii / $localGroupCount * 100)
			
			$groupObj = [ADSI]( "WinNT://$computer/$localGroup" )
			...
			Write-Progress -Activity 'Adding user' -Status 'Complete' -Id 2 -ParentId 1 -Completed

			$ii++
		}
		Write-Progress -Activity 'Adding users to the local group' -Status 'Complete' -Id 1 -ParentId 0 -Completed

		$i++
	}
	Write-Progress -Activity 'Adding users to local groups on the computer' -Status 'Complete' -Id 0 -Completed
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC+zLsjIS4NbwGl
# XgoQXCeTV2Ys+tPaCmOXxxZslCZn86CCFJQwggo1MIIIHaADAgECAgRRsGn6MA0G
...
# CQQxIgQggMxiUWznEiUbDw7VvR11r1K+CCc0tvidGgTjgRqdgxowDQYJKoZIhvcN
# AQEBBQAEggEAhXPur1vfp2eLFpApuzoRoM/YETl1HUbid6FWSnztjpiWeCQKaGed
# oN1DT8QhxbRGJnFQ0zZeXjvPWHZKnkVn95GxPPMwoBAmgBJwW+z9lJAbMvrQhfux
# gzTP0ArNP+wF+32sfpw87CicFFInH50CLm3wtL+TeXQk3h5Ik6YgebZFigCnRijj
# UR0YnVvEbHNxBDeUOHMWi94nsTWbtBIxUXx+uSPOlx6IT25tVrW+ODV023lMfpLx
# 0ysDTCUPH+/LL/n7ykKlkCiOlnKRPDISlHsibi/n18paBRNf1LsxahldRjNkMhYq
# KZT9VJg/wIOcxK4aD66PEa/GB8Qpbw0MLQ==
