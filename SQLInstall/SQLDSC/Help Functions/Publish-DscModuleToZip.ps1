<#
    .SYNOPSIS
        Publishes a DSC module to the supplied directory in the correct zip format for consumption by DSC

    .DESCRIPTION
        Scans the supplied command for modules used and determines if the module is in the supplied module source path. If the module does not exist, the function locates the module on the local system, zips it up, and publishes it to the module source path.

    .PARAMETER CommandDefinition
        The command definition in which the modules are used.

    .PARAMETER ModuleSourcePath
        The path where the modules are published for DSC consumption.

    .EXAMPLE
        $command = Get-Command -Name SQLLocalConfigurationManager
        Publish-DscModuleToZip -CommandDefinition $command.Definition -ModuleSourcePath '\\server01\Modules'
#>
function Publish-DscModuleToZip
{
    [CmdletBinding()]
    param
    (
    ...
    # Break the command definition into an array so we can look for Import-Module more easily
    [array]$lines = $CommandDefinition -split [System.Environment]::NewLine

    foreach ( $line in $lines )
    {
        # Get the parameters passed to the function
        $importDscResourceParameters = @{}

        # Determine which parameters have been passed
        switch -Regex ( $line )
        {
            # Name (positional parameter)
            '^(\s+)Import-DscResource(\s+(\w+))(\s+)?$'
            {
                $importDscResourceParameters.Add('Name',$Matches[3])
            }

            # Name
            '^(\s+)Import-DscResource(\s+-Name\s+(\w+))(\s+)?$'
            {
                $importDscResourceParameters.Add('Name',$Matches[3])
            }

            # Name, ModuleName
            '^(\s+)Import-DscResource(\s+-Name\s+(\w+))(\s+-ModuleName\s+([\d\.]+))(\s+)?$'
            {
                $importDscResourceParameters.Add('Name',$Matches[3])
                $importDscResourceParameters.Add('ModuleName',$Matches[5])
            }

            # ModuleName, Name
            '^(\s+)Import-DscResource(\s+-ModuleName\s+(\w+))(\s+-Name\s+([\d\.]+))(\s+)?$'
            {
                $importDscResourceParameters.Add('ModuleName',$Matches[3])
                $importDscResourceParameters.Add('Name',$Matches[5])
            }

            # ModuleName, ModuleVersion
            '^(\s+)Import-DscResource(\s+-ModuleName\s+(\w+))(\s+-ModuleVersion\s+([\d\.]+))(\s+)?$'
            {
                $importDscResourceParameters.Add('ModuleName',$Matches[3])
                $importDscResourceParameters.Add('ModuleVersion',$Matches[5])
            }

            # ModuleVersion, ModuleName
            '^(\s+)?Import-DscResource(\s+-ModuleVersion\s+([\d\.]+))(\s+-ModuleName\s+(\w+))(\s+)?$'
            {
                $importDscResourceParameters.Add('ModuleName',$Matches[5])
                $importDscResourceParameters.Add('ModuleVersion',$Matches[3])
            }
        }

        if ( $importDscResourceParameters.Count -gt 0 )
        {
            # Define the parameters to pass into Get-DscResource
            $getDscResourceParameters = @{}
            $getDscResourceModuleVersion = ''

            foreach ( $importDscResourceParameter in $importDscResourceParameters.GetEnumerator() )
            {
                
                $getDscResourceParameterName = ''

                if ( $importDscResourceParameter.Key -eq 'ModuleVersion' )
                {
                    $getDscResourceModuleVersion = $importDscResourceParameter.Value
                    continue
                }
                elseif ( $importDscResourceParameter.Key -eq 'ModuleName' )
                {
                    $getDscResourceParameterName = 'Module'
                }
                else
                {
                    $getDscResourceParameterName = $importDscResourceParameter.Key
                }

                $getDscResourceParameters.Add($getDscResourceParameterName,$importDscResourceParameter.Value)
            }

            # Get the DSC resources that match the supplied parameters
            $dscResource = Get-DscResource @getDscResourceParameters
            if ( -not [System.String]::IsNullOrEmpty($getDscResourceModuleVersion) )
            {
                $dscResource = $dscResource | Where-Object -FilterScript { $_.Version -eq $getDscResourceModuleVersion }
            }

            # Get the module the DSC resources live in
            $module = $dscResource | Select-Object -ExpandProperty Module -Unique

            # Determine the full path and name of the zip file
            $moduleZipFullPath = Join-Path -Path $ModuleSourcePath -ChildPath "$($module.Name)_$($module.Version.ToString()).zip"

            # Zip up the module
            if ( -not ( Test-Path -Path $moduleZipFullPath -PathType Leaf ) )
            {
                $compressArchiveParameters = @{
                    Path = Join-Path -Path $module.ModuleBase -ChildPath '*'
                    DestinationPath = $moduleZipFullPath
                }
                Compress-Archive @compressArchiveParameters
            }

            # Create a checksum of the zipped module
            if ( -not ( Test-Path -Path "$($moduleZipFullPath).checksum" -PathType Leaf ) )
            {
                New-DscChecksum -Path $moduleZipFullPath -OutPath $ModuleSourcePath
            }
        }
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCThtj2I9XdJFBT
# OVG+ViDrThv5eg1NLqwzdZ3YLmyeQqCCFJQwggo1MIIIHaADAgECAgRRsGn6MA0G
...
# CQQxIgQgB3J3pzraeqBqHIDmpgNMxyhcPVPCdmAQ9LVHPGdfW+0wDQYJKoZIhvcN
# AQEBBQAEggEAtmuf/wM+JBd4d9BMa8dlaA01KeCXotkWCQEUzxLGGHHG52L415Cr
# 8jOlZFtf/fR1C4BHJ8mUNX3frLGu3Q7tTcw/HoJcDQk+oKaFtbuqfirUVSzmGaVf
# +ppzDVVmE0BM2yLJQAKyb6FSkTnckGYGZa8SQN/BxZ9SRJaXmP0P8CFTB0+g8vDc
# 7XBZAxQg9vOkOgArE7Qqw4/kqFgLTctgaPjMlAdK61LYoLAM4oqVlJ1q7AqA9ASv
# mzuQ39DvcniEnfSLzHwxGu8P3hjf6mR2GBsJjVI35QlO+2yqAy+IdnGKH/0BqBeQ
# mkcmCmx2s3rY7ItY2GOSUQQ/FceMAIxNuQ==
