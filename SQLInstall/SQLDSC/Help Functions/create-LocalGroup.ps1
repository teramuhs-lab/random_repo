...
#>
    ...
    (
        ...
        #description of the Group to create
	...
	Write-Progress -Activity 'Creating local groups' -Status 'Initializing' -Id 0
	
	# Set up some counters
	$computerCount = $ComputerName.Count
	$localGroupCount = $LocalGroupName.Count
	

	$i = 1
	foreach ( $computer in $ComputerName )
	{
		Write-Progress -Activity 'Creating local groups on computer' -Status $computer -Id 0 -PercentComplete ($i / $computerCount * 100)

		$ii = 1
		foreach ( $localGroup in $LocalGroupName )
		{
			Write-Progress -Activity 'creating local group' -Status $localGroup -Id 1 -ParentId 0 -PercentComplete ($ii / $localGroupCount * 100)
			
            if ( [ADSI]::Exists("WinNT://$($computer)/$($localGroup)") -eq $true ) {
                
                write-host " `'$($localGroup)`' group already exists on computer: $($computer)  " -ForegroundColor Yellow
			
                }
            else {
                write-host " `'$($localGroup)`' group does not exist on computer: $($computer). Creating it now... " -ForegroundColor Yellow

                $groupObj = [ADSI]( "WinNT://$computer" )
			
                $Group = $groupObj.Create('Group', $localGroup) 
                $Group.SetInfo() 			
                $Group.Description  = $GroupDescription

                $Group.SetInfo()   
                write-host " `'$($localGroup)`' is successfully created on computer: $($computer). "
                } 


			$ii++
		}
		Write-Progress -Activity 'Creating local group' -Status 'Complete' -Id 1 -ParentId 0 -Completed

		$i++
	}
	Write-Progress -Activity 'Creating local groups on the computer' -Status 'Complete' -Id 0 -Completed
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBnEflJ8HcUAm9n
# US3baEZu3VYoemFyKH36R+XQ2fsp8qCCFJQwggo1MIIIHaADAgECAgRRsGn6MA0G
...
# CQQxIgQgzQaCuYgE1EtYQjq3J275pCWY1t03XJouk+uhKXAD3qswDQYJKoZIhvcN
# AQEBBQAEggEAW6L4AWj5iQWEA1Iqm1ZR/kLf4B0Z/CbeqNnobONTHDCMVxc/BDvh
# msKrCdPlzZt4xSCHnPtNIh79s47YTQoKICatUkDNZwqYJXt5Lyw5b7fywnZ2vF8u
# KBYhd5uJuAkaM+BEmWOXS4wcrEcb4B0qou8PdMVkqE6sj10EVF1XHkwX5y/LqLXE
# c/O1gkJosdeCNEscAtR5CygiVg7Ga2rTMld7LO7jDjZGJ/nULigHxiE6QwSEcNcJ
# adqNjImfUtTuPMfDJszBKMO+ZApkv8oYDThHUYNOAbMbAk2FHg378I4w5l/pW5PL
# Fx4ptQp85oyM2itRt77pbpCsE9DSs5sHOw==
