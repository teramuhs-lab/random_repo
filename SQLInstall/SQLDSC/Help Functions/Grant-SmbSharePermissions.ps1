...
#>
	...
	(
		...
		[ValidateScript({ ($_.FullName -as [URI]).IsUnc })]
		...
		[Parameter(Mandatory=$true,ParameterSetName='User')]
		[Parameter(ParameterSetName='Computer')]
		[Microsoft.PowerShell.Cmdletization.GeneratedTypes.SmbShare.ShareAccessRight]$ShareAccessRight,
	...
	# Set up a script block that will be executed on the share server
	$aclScriptBlock = {
		Param
		(
			[System.IO.DirectoryInfo]$Path,
		...
		foreach ( $acct in $Account )
		{
			...
			$ace = New-Object System.Security.AccessControl.FileSystemAccessRule($acct,$rights,$inherit,$propagate,$access)
	...
	# Format the accounts for use later
	switch ($PsCmdlet.ParameterSetName)
	{
		...
		'Computer'
		{
			[array]$accounts = $Computer | % { $_ + '$' }
		}
	}
	
	# Get the share info
	if ( $Path.FullName -match '^\\\\([\w-]*)\\(\w*)' )
	{
		$shareServer = [System.Net.Dns]::GetHostByName($Matches[1]).HostName
		$shareName = $Matches[2]
	}
	else
	{
		throw "Unable to determine the share information from $($Path.FullName)"
	}

	# Create the sessions we need to the share server
	$shareServerCIMSession = New-CimSession -ComputerName $shareServer
	...
	Invoke-Command -Session $shareServerPsSession -ScriptBlock $aclScriptBlock -ArgumentList $shareInfo.Path,$accounts,$FileSystemRights
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAV2NEgyJqNa+RS
# 5GALu48vCsqORxebxZ9WWHnbzfeRyqCCFJQwggo1MIIIHaADAgECAgRRsGn6MA0G
...
# CQQxIgQgNpyEMq9s621vCZe1S4y9YB0ulg5/r9/LiSSwRKXweyswDQYJKoZIhvcN
# AQEBBQAEggEAIup0VQdZpZh9hxOLQ8RGppD28zO9o9xKUZlr7N9NgizU6iopohu9
# ub1FcOaWlNczeoKaevYU0nlr/c12AI9EOXuq4w5l963TyWRz0zzcDfblA5HRR0rs
# laW3N7+gQszq8d+TQkINQcbCfBtgKQwAF1PpDN8maSnVNXVemXma0FeUPOntwK7s
# e13c+eG2CTi+yfVUrxIewgStZSsuYSu8U7gXIRB1ts3AhPE6csYSM6de8RLaiTyC
# exw+awX6EPhADsVQtoHPp5uAPLnwuxYt1ecUn6d3a7e09BmQMBQvCzww5RkW+4ON
# OB76QG0U/UjJNEjFwSbnAo4jVQY20pwNmg==
